...
`.env` 파일을 `.gitignore`에 추가해서 혹시나 실수로라도 이 토큰값들을 모든 사람이 볼 수 있는 오픈 소스 리포지터리에 올리는 일이 없도록 주의합니다. 매우 중요한 부분이니 유의하세요. 만약 다른 관리자들이 로컬 환경에서 패키지를 배포해야 한다면 그들은 아래 과정을 통해 따로 본인만의 `.env` 파일을 설정해야 합니다. (이후에, 풀 리퀘스트를 메인 브랜치에 합쳤을 때 자동으로 배포하는 방법도 알아볼 것입니다.)
...
이 명령어는 우리가 여태까지 입력한 모든 커밋에 대한 굉장히 긴 changelog 리스트를 만듭니다. 만약 우리가 메인 브랜치에 계속 푸쉬를 하고 있었다면, 하지 말라는 경고를 합니다.
