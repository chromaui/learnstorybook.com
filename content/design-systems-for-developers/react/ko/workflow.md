...
디자인팀과 개발팀이 이루고자 하는 목표의 가치는 프런트엔드 도구들이 어떻게 함께 작동하느냐에 영향을 많이 받습니다. 잘 구성되었다면 UI 컴포넌트를 개발하고 재사용하는 과정이 원활해야 합니다.
...
온라인 스토리북은 팀이 함께 공유하는 공통 레퍼런스입니다. `AvatarList` 링크를 다른 관계자들과 공유해서 더 빠르게 피드백을 받을 수 있다. 개발 환경을 만들 필요도 없고 코드를 건드릴 필요도 없으니 팀원들이 당신을 사랑할 것입니다. 
...
테스트가 성공적으로 반영되었습니다. 나중에 회귀 테스트를 하는 사람은 디자인 시스템을 파고드느라 고생을 꽤 할 것입니다.
...
풀 리퀘스트에 `minor` 레이블을 추가합시다. 이 레이블은 풀 리퀘스트가 머지되면 `Auto`에게 패키지의 minor 버전을 업데이트하도록 지시합니다.
