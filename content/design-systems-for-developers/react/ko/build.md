...
챕터 3에서 우리는 가장 인기 있는 컴포넌트 탐색기인 Storybook을 이용하여 필수 디자인 시스템 제작과정을 설정할 것입니다. 이 가이드의 목표는 전문적인 팀들이 어떻게 디자인 시스템을 구축하는지 보여주는 것입니다. 그러므로 우리는 깔끔한 코드, 시간을 절약해주는 Storybook 애드온, 디렉터리 구조 등과 같은 세부 사항들을 살펴볼 것입니다.
...
새로운 story를 선택하고 어떻게 컴포넌트가 동작하는지와 일관성을 유지하는지 확인해 보세요. 한 발 더 나아가 디자인 시스템을 사람의 인터렉션(interaction)에 의존하지 않는, 보다 강력하고 버그가 없는 상태로 만들 수 있습니다.
