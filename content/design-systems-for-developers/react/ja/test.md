...
第 5 章では、UI バグを防ぐためにデザインシステムのテストを自動化します。本章で UI コンポーネントのどのような特徴がテストを保証するのか、潜在的な落とし穴を避けるのかについて掘り下げます。包括的な範囲、分かりやすいセットアップ、コストの低いメンテナンスに釣り合いを取るテスト戦略を手に入れるため Wave、BBC、Salesforce の専門家チームを調査しました。
