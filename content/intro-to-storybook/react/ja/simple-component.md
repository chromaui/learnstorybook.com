...
title: '単純なコンポーネントを作る'
tocTitle: '単純なコンポーネント'
description: '単純なコンポーネントを独立して作りましょう'
commit: '3d9cd8c'
---
...
`Task` の作成を始めるにあたり、事前に上記のそれぞれのタスクに応じたテスト用の状態を作成します。次いで、モックデータを使用し、Storybook を使い、コンポーネントを独立した環境で作ります。それぞれの状態について、コンポーネントの外観を目で確認しながら進めます。
...
<div class="aside">
<a href="https://storybook.js.org/addons/introduction/#2-native-addons"><b>action アドオン</b></a>は独立した環境で UI コンポーネントを作成するのに役立ちます。独立した環境ではよく、アプリケーションのコンテキストや状態にアクセスできないためです。<code>action()</code> はそのスタブとして使用できます。
</div>
...
見た通り、独立した環境でコンポーネントを作り始めるのは早くて簡単です。あらゆる状態を掘り下げてテストできるので、高品質で、バグが少なく、磨かれた UI を作ることができることでしょう。
