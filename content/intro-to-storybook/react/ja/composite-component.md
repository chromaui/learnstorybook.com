...
コンポーネントが大きくなるにつれ、入力で必要となるデータも増えていきます。`TaskList` のプロパティを定義しましょう。`Task` が子供のコンポーネントなので、`Task` を表示するのに必要なデータが入力されることを確認しましょう。時間を節約するため、前の章で `Task` に定義した `propTypes` を再利用しましょう。
