...
컴포넌트의 주요 작업은 prop들을 받아서 한 조각의 UI으로 렌더링하는 것이다. 더 복잡한 컴포넌트들은 application의 상태를 추적하기도 하며 application의 동작들을 컴포넌트 트리에 전달한다.
...
상호 작용 테스트 워크플로우(workflow)는 다음과 같다.

<!-- 
Here's what the interaction testing workflow looks like: -->

1.  **📝 구성:** 컴포넌트를 분리해 적절한 prop를 주고 초기상태로 만든다.
...
Taskbox 앱은 [Jest](https://jestjs.io/)와 함께 사전 구성된 Create React App을 사용해 부트스트랩되었다. 여기서 우리는 테스트를 쓰고 실행하는데 이 앱을 사용하려 한다.
...
### 컴포넌트가 무엇을 하는지 테스트해라(하자), 어떻게 하는지 말고.
...
유닛 테스트와 마찬가지로, 컴포넌트의 내부 작동테스트도 피하고 싶다. 내부 작동테스트는 코드를 리팩터링 할 때마다 결과값의 변화 여부에 관계없이 테스트를 중단시키므로 테스트를 불안정하게 만든다. 그래서 속도가 느려진다.
...
Much like unit tests, we want to avoid testing the inner workings of a component. This makes tests brittle because any time you refactor code it'll break the tests, regardless of whether the output changed or not. Which in turn slows you down. -->
 ...
 그게 바로 Adobe, Twilio, Gatsby 그리고 더 많은 회사의 팀들이 [Testing-Library](https://testing-library.com/)를 사용하는 이유이다.[Testing-Library]은 렌더링된 결과물을 평가할 수 있게 한다.
...
우리는 컴포넌트의 내부 상태와 메소드들에 접근하는 대신, real-world 사용법을 모방한 테스트를 작성할 수 있다. 더불어 유저의 관점에서 테스트를 작성하는 것은 우리의 코드가 동작한다는 사실에 더 많은 자신감을 준다.
...
앞 장에서는 'InboxScreen.stories.js' 파일에 'InboxScreen' 구성 요소의 모든 사용 사례를 목록으로 작성했다.이를 통해 개발 중 모습을 확인하고 비주얼 테스트를 통해 regression(회귀,퇴행..)을 포착할 수 있었다. 
이 story들은 이제 우리의 상호 작용 테스트에도 힘을 실어줄 것이다.
...
```
스토리는 표준 자바스크립트 모듈을 기반으로 하며 이동가능한 형식으로 작성된다. 
어떤 자바스크립트 기반의 테스트 라이브러리(Jest, Testing Lib, Playwright)와도 함께 스토리들을 재사용할 수 있다.
따라서 제품군의 각 테스트 도구에 대한 테스트 사례를 설정하고 유지관리할 필요가 없다. 예를 들어, Adobe Spectrum 디자인설계 팀은 이러한 패턴을 사용해 메뉴와 대화 상자 컴포넌트에 대한 [상호작용 테스트]
(https://github.com/adobe/react-spectrum/blob/f6c06605243ad2033fce95f80ae3fecd4a38daeb/packages/%40react-spectrum/dialog/test/DialogContainer.test.js#L62)를 수행한다. 
...
그 'it' 블록은 우리의 테스트를 나타낸다.먼저 컴포넌트를 렌더링하고 데이터를 가져올 때까지 기다린 후 특정 작업을 찾아 핀 버튼을 클린한다. 확언 문장은 고정된 상태가 업데이트되었는지 확인한다. 마지막으로 `afterEach` 블록이 테스트 중에 마운트된 리액트 트리들을 마운트 해제하여 정리한다.
...
```
'yarn test'실행해 Jest를 시작한다.
그러면 테스트가 실패했음을 알 수 있을 것이다.
...
`InboxScreen`은 백엔드에서 데이터를 가져온다. [이전 챕터](../composition-testing)에서는,
이러한 API 요청을 가작업하기 위해 [Storybook MSW addon](https://storybook.js.org/addons/msw-storybook-addon/)를 설치했다.
하지만 Jest에서는 이 방식이 통하지 않는다. 
우리는 이 요청과 다른 컴포넌트 의존성을 함께 가져올 방법이 필요하다.
...
복잡한 구성요소들은 테마 프로바이더나 컨텍스트와 같은 외부 의존성에 의존해 글로벌 데이터를 공유한다. Storybook은 [decorators](https://storybook.js.org/docs/react/writing-stories/decorators)을 사용해 스토리를 감싸고 그러한 기능을 제공한다. 모든 구성과 함께 스토리를 가져오려면 [@storybook/testing-react](https://github.com/storybookjs/testing-react) 라이브러리를 사용한다.
...
이 과정은 보통 두 단계로 이루어진다. 
우선 글로벌 데코레이터들을 모두 등록해야한다.
우리의 경우, 우리는 두 데코레이터를 가진다:
Chakra UI 테마를 제공하는 데코레이터와 
MSW addon을 위한 데코레이터. 둘 다 [`.storybook/preview`](https://github.com/chromaui/ui-testing-guide-code/blob/interaction-testing/.storybook/preview.js) file 파일을 통해 구성된다.
...
Jest는 프로젝트가 부트스트랩될때 CRA에 의해 자동으로 생성되는 글로벌 설정 파일 'setupTest.js를 제공한다. 해당 파일을 업데이트해 Storybook의 글로벌 설정을 등록한다.
...
이어서, 테스트를 업데이트하여 `@storybook/testing-react`에서 `composeStories` 유틸리티를 사용한다.
이 테스트는 모든 데코레이터가 적용된 스토리들을 1:1 map으로 치환한다. 
그러면 짜잔, 우리의 테스트는 통과했다!
...
```
요약하면, 설정 코드는 스토리안에 존재하고, 액션과 명령들은 테스트 파일안에 존재한다. 코드들은 Testing Library을 사용해 사용자의 방식으로 UI와 상호작용한다.
향후 컴포넌트 구현이 변경되면, 결과물이나 동작이 수정될 때에만 테스트가 중단될 것이다.
...
모든 사용자가 당신의 UI를 사용할 수 있게되면,비즈니스 재무에 영향을 미치고 [법적 요구사항](https://www.w3.org/WAI/policies/?q=government)을 충족시키게 된다.이것은 윈-윈 관계이다. 
다음 장에서는 스토리들의 이동성을 활용해 접근성을 간소화하는 방법을 보여주려 한다.
